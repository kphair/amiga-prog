...
int FileArea_listviewClicked( void ) 
{
    ...
    if(SelectedArea!=~0) {
        ...
        GT_SetGadgetAttrs(File_Area_ManagerGadgets[GD_FileAreaUsers_button],File_Area_ManagerWnd,NULL, GA_Disabled,TRUE, TAG_DONE);
...
int FileAreaNumberClicked( void ) { 
    ...
    int i; 
    BOOL sortswapped;
    SHORT oldareanum, newareanum, lastareanum, tempareanum;
    ULONG templong;
    ...
    if((SelectedArea!=~0) && (newareanum!=10000)) {
        ...
        do {
            ...
            for (i=0; i<lastareanum-1; i++) {
                ...
                if ((areanode->nn_Area.Number) > (nextareanode->nn_Area.Number)) {
                    ...
                    Insert((struct List *)FileAreaList_list,(struct Node *)areanode,(struct Node *)nextareanode);
        ...
        GT_SetGadgetAttrs(File_Area_ManagerGadgets[GD_FileArea_listview],File_Area_ManagerWnd, NULL,
                          GTLV_Labels, (ULONG)FileAreaList_list,
                          GTLV_MakeVisible, i,
                          GTLV_Selected, i,
                          TAG_DONE);
...
int InsertFileArea_buttonClicked( void ) {      /* "Insert" clicked */
    ...
    if(SelectedArea!=~0) {
        ...
        if ((nextareanode->nn_Area.Number)!=(areanode->nn_Area.Number+1) || (nextareanode==NULL) && (areanode->nn_Area.Number!=9999)) {
            ...
            Insert((struct List *)FileAreaList_list,(struct Node *)newareanode,(struct Node *)areanode);
...
int FileAreaUp_buttonClicked( void ) 
{
    ...
    if(SelectedArea!=~0 && SelectedArea!=0) {
        ...
        Insert((struct List *)FileAreaList_list,(struct Node *)prevareanode,(struct Node *)areanode);
                    ...
                    *sptr=0;
                    ...
                    for (i=lastareanum; i>=0; i--) {
                        ...
                        if (i==0 && (newareanode->nn_Area.Number != areanode->nn_Area.Number)) {
                            AddHead((struct List *)FileAreaList_list,(struct Node *)newareanode);
...
int FileApplyTemplate_buttonClicked( void )
{
    ...
    if (!OpenFileAreaTemplateWindow()) {
        ...
        SetWindowPointer(File_Area_ManagerWnd,
                         WA_BusyPointer, TRUE,
                         TAG_DONE);
...
int FileTemplateSrc_listviewClicked( void ) {   /* routine when gadget "" is clicked. */
    return(TRUE);
}
int FileTemplateDest_listviewClicked( void ) {  /* routine when gadget "" is clicked. */
    return(TRUE);
}
int FileTemplateApply_buttonClicked( void ) {   /* routine when gadget "APPLY" is clicked. */
    return(TRUE);
}
int FileTemplateCancel_buttonClicked( void ) {  /* routine when gadget "Cancel" is clicked. */
    return(TRUE);
}
int FileTemplateConfirmApply_checkClicked( void ) { /* routine when gadget "Confirm each application of template area" is clicked. */
    return(TRUE);
}
